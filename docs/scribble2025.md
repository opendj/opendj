...
Or macos not doing V6 lookups? opend-dj on RHEL has same issue, so probably more fritz box or 1&1 dsl provider issue
AWS CNAME does sometomes not resolve?

dig AAAA dev.opendj.io @8.8.8.8 

Workaround by pointing /etc/resolv.conf to google dns before starting mycroshift
```
[root@opendj-dev ~]# cat /etc/resolv.conf 
